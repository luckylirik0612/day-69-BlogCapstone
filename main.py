...
def register():
    ...
    if form.validate_on_submit():
        if db.session.query(User).filter_by(name=request.form.get("name")).first():
            is_user = True
        else:
            is_user = False
        if not is_user:
...
def login():
    ...
    form = LoginForm()
    if form.validate_on_submit():
        loged_user = db.session.query(User).filter_by(name=request.form.get("name")).first()
        if loged_user:
            if check_password_hash(loged_user.password, request.form.get("password")):
